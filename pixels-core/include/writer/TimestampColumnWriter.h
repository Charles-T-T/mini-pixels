...
class TimestampColumnWriter : public ColumnWriter {
  public:
    TimestampColumnWriter(std::shared_ptr<TypeDescription> type,
    ...
    void writeCurPartTimestamp(std::shared_ptr<ColumnVector> columnVector,
                               long *values, int curPartLength,
                               int curPartOffset);
